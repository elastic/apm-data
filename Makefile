...
fmt:
	go run golang.org/x/tools/cmd/goimports@v0.31.0 -w .
...
fieldalignment:
	go run golang.org/x/tools/go/analysis/passes/fieldalignment/cmd/fieldalignment@v0.31.0 -test=false $(shell go list ./... | grep -v modeldecoder/generator | grep -v test | grep -v model/modelpb)

update-licenses:
	go run github.com/elastic/go-licenser@v0.4.2 -ext .go .
	go run github.com/elastic/go-licenser@v0.4.2 -ext .proto .
