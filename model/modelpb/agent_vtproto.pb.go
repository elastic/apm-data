...
func (m *Agent) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ActivationMethod) > 0 {
		i -= len(m.ActivationMethod)
		copy(dAtA[i:], m.ActivationMethod)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ActivationMethod)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.EphemeralId) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.EphemeralId)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Version) > 0 {
...
func (m *Agent) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 4:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ActivationMethod", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ActivationMethod = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
