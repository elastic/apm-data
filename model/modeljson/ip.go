...
func ParseIP(s string) (*modelpb.IP, error) {
...
func MustParseIP(s string) *modelpb.IP {
	ip, err := ParseIP(s)
	if err != nil {
		panic(err)
	}
...
func Addr2IP(addr netip.Addr) *modelpb.IP {
	if addr.Is4() {
		return &modelpb.IP{
...
func IP2Addr(i *modelpb.IP) netip.Addr {
...
func IP2String(i *modelpb.IP) string {
	addr := IP2Addr(i)
	if addr.IsValid() {
		return addr.String()
	}
