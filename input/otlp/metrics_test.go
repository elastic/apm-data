...
func TestConsumeMetrics(t *testing.T) {
	...
	expected := []*modelpb.APMEvent{{
		Agent:     &agent,
		Service:   &service,
		Timestamp: modelpb.TimeToPBTimestamp(timestamp0),
		...
		Agent:     &agent,
		Service:   &service,
		Timestamp: modelpb.TimeToPBTimestamp(timestamp1),
		Metricset: &modelpb.Metricset{
			Name: "app",
			Samples: []*modelpb.MetricsetSample{
...
func TestConsumeMetricsExportTimestamp(t *testing.T) {
	...
	assert.InDelta(t, modelpb.TimeToPBTimestamp(now.Add(dataPointOffset)), events[0].Timestamp, float64(allowedError.Nanoseconds()))
...
func eventsMatch(t *testing.T, expected []*modelpb.APMEvent, actual []*modelpb.APMEvent) {
	...
	const allowedError = 2 * time.Second

	now := modelpb.PBTimestampNow()
	for i, e := range actual {
		assert.InDelta(t, now, e.Event.Received, float64(allowedError.Nanoseconds()))
		e.Event.Received = 0
